...
def price_dict(d: dict) -> callable:
    """Wrap a bundle→price dict as a callable for ceca_step."""
...
def test_ceca_step_satisfied_via_cached_belief_no_llm_call():
    proxy = make_proxy(['{"bundle_value": 100}'], epsilon=1.0)
    proxy.infer_cached_xor_bid([frozenset({"A"})])
    calls_before = len(proxy.person.client.calls)

    response = proxy.ceca_step(
        prices=price_dict({frozenset({"A"}): 100.0}),
        current_bundle=frozenset({"A"}),
        round_idx=0,
    )

    assert response.satisfied is True
    assert response.demanded_bundle is None
    assert response.value is None
    # The proxy's own cached belief already agrees current_bundle is best,
    # so no LLM call is issued at all.
    assert len(proxy.person.client.calls) == calls_before


def test_ceca_step_person_reports_satisfied_despite_stale_belief_no_atom_update():
    proxy = make_proxy(
        [
            '{"bundle_value": 100}',
            '{"satisfied": true, "preferred_bundle": null}',
    ...
    # current_bundle (empty) has surplus 0; cached {A} has surplus 100, so
    # the proxy's own belief disagrees and triggers an LLM ceca_demand_query.
    response = proxy.ceca_step(
        prices=price_dict({}),
    ...
    assert response.satisfied is True
    assert response.demanded_bundle is None
    # The person's satisfied=True is trusted with no atom update.
    assert cached_bid.atoms == [XorAtomicBid(bundle=frozenset({"A"}), value=100.0)]


def test_ceca_step_unsatisfied_with_alternative_upserts_preferred_bundle():
    proxy = make_proxy(
        [
            '{"bundle_value": 100}',
            '{"satisfied": false, "preferred_bundle": ["B"]}',
    ...
    response = proxy.ceca_step(
        prices=price_dict({frozenset(): 100.0}),
        current_bundle=frozenset(),
        round_idx=0,
    )
...
def test_ceca_step_unsatisfied_without_alternative_falls_back_to_proxy_best_bundle():
    proxy = make_proxy(
        [
            '{"bundle_value": 100}',
            '{"satisfied": false, "preferred_bundle": null}',
    ...
    assert response.satisfied is False
    assert response.demanded_bundle == frozenset({"A"})
    assert response.value == 90.0


def test_ceca_step_pruning_drops_redundant_item():
    proxy = make_proxy(
        [
            '{"bundle_value": 100}',
            '{"satisfied": false, "preferred_bundle": ["A", "B"]}',
            '{"bundle_value": 200}',
            '{"bundle_value": 200}',
        ],
        epsilon=1.0,
    ...
    response = proxy.ceca_step(
        prices=price_dict({}),
        current_bundle=frozenset({"C"}),
        round_idx=0,
    )

    # {A,B} priced at 200 but {B} alone is also worth 200 -- A is pruned.
    assert response.satisfied is False
    assert response.demanded_bundle == frozenset({"B"})
    assert response.value == 200.0
    assert proxy.pruning_query_count == 1
    assert XorAtomicBid(bundle=frozenset({"B"}), value=200.0) in cached_bid.atoms
    assert not any(atom.bundle == frozenset({"A", "B"}) for atom in cached_bid.atoms)


def test_ceca_step_raises_without_cached_bid():
    proxy = make_proxy([], epsilon=1.0)

    with pytest.raises(RuntimeError, match="No cached XOR bid"):
        proxy.ceca_step(prices=price_dict({}), current_bundle=frozenset(), round_idx=0)
