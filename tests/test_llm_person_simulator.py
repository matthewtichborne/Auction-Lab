...
def test_ceca_demand_query_satisfied_returns_parsed_response():
    client = MockLlmClient(['{"satisfied": true, "preferred_bundle": null}'])
    simulator = make_simulator(client)

    response = simulator.ceca_demand_query(
        frozenset({"IPAD"}), bundle_prices={frozenset({"IPAD"}): 100.0}
    )

    assert response.satisfied is True
    assert response.preferred_bundle is None
    assert "Price of your current bundle: 100.0" in client.calls[0]


def test_ceca_demand_query_unsatisfied_returns_preferred_bundle():
    client = MockLlmClient(
        ['{"satisfied": false, "preferred_bundle": ["PENCIL"]}']
    )
    simulator = make_simulator(client)
...
def test_ceca_demand_query_retries_on_parse_failure():
    client = MockLlmClient(
        [
            "not json",
            '{"satisfied": true, "preferred_bundle": null}',
...
def test_ceca_demand_query_writes_log_with_distinct_prompt_type(tmp_path):
    log_path = tmp_path / "calls.jsonl"
    client = MockLlmClient(['{"satisfied": true, "preferred_bundle": null}'])
    simulator = LlmPersonSimulator(
        bidder_id="bidder_1",
    ...
    simulator.ceca_demand_query(
        frozenset({"IPAD"}), bundle_prices={frozenset({"IPAD"}): 100.0}
    )

    record = json.loads(log_path.read_text().strip())
    assert record["prompt_type"] == "ceca_demand_query"
