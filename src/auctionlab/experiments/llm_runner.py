...
def make_dnf_learning_proxies_for_instance(
    *,
    instance: AuctionInstance,
    scenario_description: str,
    person_seeds: dict[str, str],
    item_descriptions: dict[str, str],
    clients: dict[str, LlmClient],
) -> dict[str, DnfLearningProxy]:
    """Construct one ωxor (DNF/proper-learning) proxy per instance bidder."""
    validate_bidder_keys(
        bidder_ids=instance.bidder_ids,
        values=person_seeds,
        label="person_seeds",
    ...
    proxies: dict[str, DnfLearningProxy] = {}
    for bidder_id in instance.bidder_ids:
        person = LlmPersonSimulator(
            bidder_id=bidder_id,
            scenario_description=scenario_description,
        ...
        proxies[bidder_id] = DnfLearningProxy(
            bidder_id=bidder_id,
            person=person,
...
def make_hybrid_proxies_for_instance(
    *,
    instance: AuctionInstance,
    scenario_description: str,
    person_seeds: dict[str, str],
    ...
    alpha: int = 10,
    delta: float = 0.95,
) -> dict[str, HybridProxy]:
    """Construct one ωh (hybrid LLM/DNF) proxy per instance bidder.

    Each bidder gets an :class:`LlmAuctionProxyAdapter` (ωnvd/ωvd-style
    NL-and-inference) and a :class:`DnfLearningProxy` (ωxor-style exact
    learning) sharing one :class:`LlmPersonSimulator`, combined via
    :class:`HybridProxy`: the first ``alpha`` refinements go to the LLM
    proxy, then refinement switches to the DNF proxy and the LLM proxy's
    remaining inferred values decay by ``delta`` each refinement.
    """
    validate_bidder_keys(
        bidder_ids=instance.bidder_ids,
        values=person_seeds,
    ...
    bundles_by_bidder, _candidate_counts, _targeted = _resolve_candidate_bundles(
        instance=instance,
        candidate_bundles=candidate_bundles,
        candidate_bundles_by_bidder=candidate_bundles_by_bidder,
    )

    proxies: dict[str, HybridProxy] = {}
    for bidder_id in instance.bidder_ids:
        person = LlmPersonSimulator(
            bidder_id=bidder_id,
            scenario_description=scenario_description,
