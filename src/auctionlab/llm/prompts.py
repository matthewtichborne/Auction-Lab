...
def build_ceca_demand_query_prompt(
    *,
    scenario_description: str,
    person_seed: str,
    item_descriptions: dict[Item, str],
    current_bundle: Bundle,
    bundle_prices: dict[Bundle, float],
) -> str:
    """Ask whether the person is satisfied with ``current_bundle`` at its
    CECA personalized bundle price.

    Unlike :func:`build_demand_query_prompt`'s per-item price vector, CECA
    prices a fixed list of named bundles individually (Lindahl-style
    personalized pricing): a bundle's price is exactly what's listed for
    it, not a sum of per-item prices, and any bundle not listed is
    implicitly free ($0) -- an exploration incentive for bundles the person
    hasn't priced yet. The prompt spells this out explicitly since an LLM
    persona's prior from any other auction context defaults to
    "market-clearing item prices," which would misread why unpriced
    bundles cost nothing here.
    """
    current_bundle_description = describe_bundle(current_bundle, item_descriptions)
    current_price = bundle_prices.get(current_bundle, 0.0)

    other_priced_lines = "\n".join(
        f"- [{','.join(sorted(bundle))}]: {price}"
        for bundle, price in sorted(
            bundle_prices.items(),
            key=lambda item: (len(item[0]), tuple(sorted(item[0]))),
        )
        if bundle != current_bundle
    )
    if not other_priced_lines:
        other_priced_lines = "(none -- every other bundle is priced at $0)"
...
PRICING RULES (read carefully -- this is NOT a per-item price list):
Each bundle below has its OWN price, set individually for this person. A
bundle's price is exactly the number listed for it -- it is NOT computed by
summing per-item prices. Any bundle not listed below (including the empty
bundle) is implicitly priced at $0: an exploration incentive so the person
can reveal a new bundle's value for free.

YOUR CURRENT BUNDLE = {sorted(current_bundle)}

Current bundle item descriptions:
{current_bundle_description}

Price of your current bundle: {current_price}

Other named bundles and their prices (for reference, if you prefer a
different bundle -- any bundle not listed here costs $0):
{other_priced_lines}

Available items (for reference, if you prefer a bundle made of different
items):
{chr(10).join(f"- {item}: {description}" for item, description in sorted(item_descriptions.items()))}

Decide whether this person would be satisfied keeping exactly their current
bundle at its listed price (i.e. it is worth at least that price to them,
and no other priced or unpriced bundle would net them more surplus). If
they would not be satisfied, report the bundle of item IDs they would
prefer instead (it may be empty, smaller, larger, or different).
