...
class LlmPersonSimulator:
    ...
    def ceca_demand_query(
        ...
        if self.ground_truth_valuations is not None:
            ...
            def _lindahl_price(b: frozenset) -> float:
                # Apply the XOR-induced Lindahl superset rule: phi(b) = max value
                # of any atom in bundle_prices whose bundle is a subset of b.
                # This is Equation 4 of Huang et al. — a flat dict.get(b, 0.0)
                # would mis-price supersets of manifest atoms as 0.
                best = 0.0
                for atom_bundle, price in bundle_prices.items():
                    if atom_bundle.issubset(b):
                        best = max(best, price)
                return best

            best_bundle = current_bundle
            best_surplus = self.ground_truth_valuations.get(current_bundle, 0.0) - _lindahl_price(current_bundle)
            for b, v in self.ground_truth_valuations.items():
                surplus = v - _lindahl_price(b)
                if surplus > best_surplus:
                    best_surplus = surplus
                    best_bundle = b
            satisfied = best_bundle == current_bundle
            preferred = None if satisfied else list(best_bundle)
            _bundle_str = "{" + ",".join(sorted(str(i) for i in current_bundle)) + "}"
            result_str = "sat" if satisfied else "unsat"
            if self.verbose:
                print(f"  {self.bidder_id:<12}  cdq  {_bundle_str}  →  {result_str}  [gt]", flush=True)
            self._last_prompt = f"[ground-truth] current_bundle={sorted(current_bundle)}"
            self._last_response_summary = (
                f"satisfied={satisfied}; preferred_bundle={sorted(preferred or [])}"
            )
            if self.logger is not None:
                ...
                self.logger.log(LlmCallRecord(
                    timestamp=current_timestamp(),
                    bidder_id=self.bidder_id,
                    prompt_type="ceca_demand_query_gt",
                    prompt=self._last_prompt,
                    raw_response=self._last_response_summary,
                    parsed_response=None,
                    success=True,
        ...
        for attempt in range(1, self.max_parse_retries + 2):
            ...
            if self.verbose:
                print(
                    f"  {self.bidder_id:<12}  cdq  {_bundle_str}{_retry}",
                    end="",
                    flush=True,
                )
            started = time.perf_counter()
        ...
        raise RuntimeError("CECA demand query attempts exhausted unexpectedly")
